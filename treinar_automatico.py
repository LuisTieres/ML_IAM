# =============================================================================
# IAM RISK PREDICTOR — Automacao de Treinamentos
# =============================================================================
# Roda N treinamentos com diferentes seeds e configuracoes,
# compara os resultados e salva automaticamente o melhor modelo.
# Metricas: Accuracy, Precision, Recall, F1, F2 (macro e por classe)
# =============================================================================
...
from sklearn.metrics import (
    ...
    roc_auc_score, f1_score, f2_score,
    fbeta_score, accuracy_score, balanced_accuracy_score,
...
def carregar_historico():
    if os.path.exists(HISTORICO_PATH):
        with open(HISTORICO_PATH, "r") as f:
            return json.load(f)
    return {"experimentos": [], "melhor": None}
...
def avaliar_completo(modelo, X_test, y_test, encoders):
    """
    Avalia o modelo com todas as metricas relevantes para classificacao de risco.

    Metricas calculadas:
    - Accuracy:  % de acertos totais
    - Precision: das previsoes de uma classe, quantas estavam corretas
    - Recall:    dos casos reais, quantos o modelo encontrou
    - F1-Score:  equilibrio entre Precision e Recall (beta=1)
    - F2-Score:  mais peso ao Recall — penaliza mais deixar passar um Alto Risco
    ...
    for i, cls in enumerate(classes):
        # Transforma em problema binario para calcular por classe
        y_bin_true = (y_test == i).astype(int)
        y_bin_pred = (y_pred == i).astype(int)

        p  = precision_score(y_bin_true, y_bin_pred, zero_division=0)
        r  = recall_score(y_bin_true, y_bin_pred, zero_division=0)
        f1 = f1_score(y_bin_true, y_bin_pred, zero_division=0)
        f2 = fbeta_score(y_bin_true, y_bin_pred, beta=2, zero_division=0)
    ...
    # ── Score Composto ────────────────────────────────────────────
    # F2 tem mais peso porque deixar passar um Alto Risco e mais critico
    # do que gerar um falso alarme
    score_composto = (
        f2_macro    * 0.35 +   # F2 macro — penaliza mais falsos negativos
        auc         * 0.30 +   # AUC-ROC — separabilidade das classes
        f1_macro    * 0.20 +   # F1 macro — equilibrio geral
        bal_acuracia * 0.15    # Acuracia balanceada — robustez a desbalanceamento
    )

    # Bonus: F2 da classe Alto Risco (mais critica)
    if "Alto" in por_classe:
        score_composto += por_classe["Alto"]["f2"] * 0.10
        score_composto /= 1.10  # Normaliza para manter escala 0-1

    return {
        # Globais
        "acuracia":          round(acuracia, 4),
        "bal_acuracia":      round(bal_acuracia, 4),
...
def imprimir_resultado(m, config_nome, n_amostras, seed, tempo, is_melhor=False):
    ...
    print(f"  {'─'*22} {'─'*8} {'─'*10}")
    print(f"  {'Accuracy':<22} {m['acuracia']:>8.4f} {'(bal)':>6} {m['bal_acuracia']:>6.4f}")
    print(f"  {'Precision':<22} {m['precision_macro']:>8.4f} {'':>6} {m['precision_weighted']:>6.4f}")
    print(f"  {'Recall':<22} {m['recall_macro']:>8.4f} {'':>6} {m['recall_weighted']:>6.4f}")
    print(f"  {'F1-Score':<22} {m['f1_macro']:>8.4f} {'':>6} {m['f1_weighted']:>6.4f}")
    print(f"  {'F2-Score (↑Recall)':<22} {m['f2_macro']:>8.4f} {'':>6} {m['f2_weighted']:>6.4f}")
    print(f"  {'AUC-ROC':<22} {m['auc_roc']:>8.4f}")
    print(f"  {'Score Composto':<22} {m['score_composto']:>8.4f}  ← criterio de selecao")

    print(f"\n  Por classe:")
    print(f"  {'Classe':<10} {'Precision':>10} {'Recall':>8} {'F1':>8} {'F2':>8}")
    print(f"  {'─'*10} {'─'*10} {'─'*8} {'─'*8} {'─'*8}")
    for cls, vals in m["por_classe"].items():
        flag = " ⚠" if cls == "Alto" else ""
        print(f"  {cls:<10} {vals['precision']:>10.4f} {vals['recall']:>8.4f} "
              f"{vals['f1']:>8.4f} {vals['f2']:>8.4f}{flag}")
    print(f"  (⚠ Alto Risco: F2 baixo = modelo deixando passar riscos altos)")
...
def rodar_experimento(config, seed, n_amostras):
    ...
    # GradientBoosting nao aceita n_jobs
    ...
    cv_f2 = cross_val_score(modelo, X_train, y_train, cv=cv,
                             scoring="f1_macro")  # proxy
    metricas["cv_f1_mean"] = round(cv_f2.mean(), 4)
    metricas["cv_f1_std"]  = round(cv_f2.std(), 4)

    return modelo, encoders, metricas, tempo
...
def main():
    ...
    print("=" * 60)
    print("  IAM RISK PREDICTOR — Automacao de Treinamentos")
    print("=" * 60)
    print(f"  Metricas: Accuracy | Precision | Recall | F1 | F2")
    print(f"  F2 tem mais peso — penaliza deixar passar Alto Risco")
    print(f"  Seeds: {len(SEEDS)} | Tamanhos: {TAMANHOS} | Modelos: {len(MODELOS_CONFIG)}")
    total = len(SEEDS) * len(TAMANHOS) * len(MODELOS_CONFIG)
    print(f"  Total experimentos: {total}")
    print("=" * 60)

    melhor_score    = historico["melhor"]["score_composto"] if historico["melhor"] else 0
    melhor_modelo   = None
    ...
    for config in MODELOS_CONFIG:
        for n_amostras in TAMANHOS:
            for seed in SEEDS:
                ...
                try:
                    modelo, encoders, metricas, tempo = rodar_experimento(
    ...
    if novos:
        # Achata por_classe para salvar no CSV
        rows = []
        for r in novos:
            row = {k: v for k, v in r.items() if k != "por_classe"}
    ...
    if melhor_modelo is not None:
        ...
        historico["melhor"] = melhor_config
        historico["experimentos"].extend(novos)
        ...
        print(f"     F2-Macro:      {melhor_config['f2_macro']}  ← principal criterio")
        print(f"     F1-Macro:      {melhor_config['f1_macro']}")
        print(f"     AUC-ROC:       {melhor_config['auc_roc']}")
        print(f"     Score Composto:{melhor_config['score_composto']}")
        if "Alto" in melhor_config.get("por_classe", {}):
            alto = melhor_config["por_classe"]["Alto"]
            print(f"     F2 Alto Risco: {alto['f2']}  ← quantos riscos altos o modelo capturou")
